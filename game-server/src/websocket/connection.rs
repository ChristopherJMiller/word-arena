...
impl Connection {
    ...
    pub fn send_message(&self, message: ServerMessage) -> Result<(), String> {
        self.sender.send(message).map_err(|_| "Connection closed".to_string())
    }
...
impl ConnectionManager {
    ...
    pub async fn authenticate_connection(&self, id: ConnectionId, user_id: String) -> Result<(), String> {
    ...
    pub async fn send_to_game_except(&self, game_id: &str, except_connection: ConnectionId, message: ServerMessage) {
        let connections = self.connections.read().await;
        for connection in connections.values() {
...
mod tests {
    ...
    async fn test_authentication_prevents_duplicate_users() {
        ...
        let result1 = manager.authenticate_connection(conn_id1, "user1".to_string()).await;
        assert!(result1.is_ok());
        
        // Second authentication with same user should fail
        let result2 = manager.authenticate_connection(conn_id2, "user1".to_string()).await;
    ...
    async fn test_authentication_cleanup_on_disconnect() {
        ...
        let _receiver = manager.create_connection(conn_id).await;
        manager.authenticate_connection(conn_id, "user1".to_string()).await.unwrap();
    ...
    async fn test_message_sending_to_nonexistent_connection() {
        ...
        let result = manager.send_to_connection(conn_id, game_types::ServerMessage::Error { 
    ...
    async fn test_game_assignment_and_messaging() {
        ...
        manager.set_connection_game(conn_id1, Some(game_id.to_string())).await;
        manager.set_connection_game(conn_id2, Some(game_id.to_string())).await;
        
        // Send message to game
        let test_message = game_types::ServerMessage::Error { message: "game_message".to_string() };
    ...
    async fn test_concurrent_connection_operations() {
        ...
        for i in 0..50 {
            ...
            let handle = tokio::spawn(async move {
                ...
                manager_clone.authenticate_connection(conn_id, format!("user_{}", i)).await.unwrap();
