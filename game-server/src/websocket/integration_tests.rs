...
async fn test_complete_matchmaking_flow() {
    ...
    ws1.send(warp::ws::Message::text(serde_json::to_string(&auth_msg1).unwrap()))
        .await;
    
    ws2.send(warp::ws::Message::text(serde_json::to_string(&auth_msg2).unwrap()))
        .await;

    // Receive auth success for both
    let auth_response1 = timeout(Duration::from_secs(1), ws1.next()).await
        .expect("Timeout waiting for auth response")
        .expect("WebSocket closed")
        .expect("WebSocket error");
    
    let auth_response2 = timeout(Duration::from_secs(1), ws2.next()).await
        .expect("Timeout waiting for auth response")
        .expect("WebSocket closed")
        .expect("WebSocket error");
    ...
    assert!(matches!(auth_msg1, ServerMessage::AuthenticationSuccess { .. }));
    assert!(matches!(auth_msg2, ServerMessage::AuthenticationSuccess { .. }));

    // Step 2: Player 1 joins queue
    let join_queue_msg = ClientMessage::JoinQueue;
    ws1.send(warp::ws::Message::text(serde_json::to_string(&join_queue_msg).unwrap()))
        .await;

    // Receive QueueJoined for player 1
    let queue_joined1 = timeout(Duration::from_secs(1), ws1.next()).await
    ...
    ws2.send(warp::ws::Message::text(serde_json::to_string(&join_queue_msg).unwrap()))
        .await;

    // Receive QueueJoined for player 2
    let queue_joined2 = timeout(Duration::from_secs(1), ws2.next()).await
        .expect("Timeout waiting for queue joined")
        .expect("WebSocket closed")
        .expect("WebSocket error");
    ...
    let countdown1 = timeout(Duration::from_secs(1), ws1.next()).await
        .expect("Timeout waiting for countdown")
        .expect("WebSocket closed")
        .expect("WebSocket error");
    
    let countdown2 = timeout(Duration::from_secs(1), ws2.next()).await
        .expect("Timeout waiting for countdown")
        .expect("WebSocket closed")
        .expect("WebSocket error");
    ...
    if let ServerMessage::MatchmakingCountdown { seconds_remaining, players_ready, total_players } = countdown_msg1 {
        assert!(seconds_remaining <= 60);
        assert_eq!(players_ready, 0); // No votes yet
        assert_eq!(total_players, 2);
    } else {
        panic!("Expected MatchmakingCountdown message, got: {:?}", countdown_msg1);
    }
    
    // Player 2 should get the same countdown info
    assert!(matches!(countdown_msg2, ServerMessage::MatchmakingCountdown { .. }));

    // Step 4: Both players vote to start
    let vote_msg = ClientMessage::VoteStartGame;
    
    ws1.send(warp::ws::Message::text(serde_json::to_string(&vote_msg).unwrap()))
        .await;
    
    ws2.send(warp::ws::Message::text(serde_json::to_string(&vote_msg).unwrap()))
        .await;

    // Both players should receive updated countdown messages after each vote
    // After player 1 votes
    let _vote_update1_p1 = timeout(Duration::from_secs(1), ws1.next()).await
        .expect("Timeout").expect("WebSocket closed").expect("WebSocket error");
    let _vote_update1_p2 = timeout(Duration::from_secs(1), ws2.next()).await
        .expect("Timeout").expect("WebSocket closed").expect("WebSocket error");

    // After player 2 votes
    let _vote_update2_p1 = timeout(Duration::from_secs(1), ws1.next()).await
        .expect("Timeout").expect("WebSocket closed").expect("WebSocket error");
    let _vote_update2_p2 = timeout(Duration::from_secs(1), ws2.next()).await
        .expect("Timeout").expect("WebSocket closed").expect("WebSocket error");

    // Step 5: Match should be found (since both players voted)
    let match_found1 = timeout(Duration::from_secs(2), ws1.next()).await
        .expect("Timeout waiting for match found")
        .expect("WebSocket closed")
        .expect("WebSocket error");
    
    let match_found2 = timeout(Duration::from_secs(2), ws2.next()).await
        .expect("Timeout waiting for match found")
        .expect("WebSocket closed")
        .expect("WebSocket error");
    ...
    if let ServerMessage::MatchFound { game_id: game_id2, players } = match_msg2 {
    ...
    let game_state1 = timeout(Duration::from_secs(2), ws1.next()).await
        .expect("Timeout waiting for game state")
        .expect("WebSocket closed")
        .expect("WebSocket error");
    
    let game_state2 = timeout(Duration::from_secs(2), ws2.next()).await
        .expect("Timeout waiting for game state")
        .expect("WebSocket closed")
        .expect("WebSocket error");
