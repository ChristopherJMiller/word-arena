...
impl ScoringEngine {
    ...
    pub fn evaluate_guess(
        ...
        letters.resize(word_chars.len().max(target_chars.len()), LetterResult {
            letter: " ".to_string(),
        ...
        for (i, &ch) in word_chars.iter().enumerate() {
            ...
            if target_letter_count.get(&ch).unwrap_or(&0) > &0 {
                ...
                let was_previously_known = previously_revealed.values()
                    .any(|prev_status| {
    ...
    pub fn determine_round_winner(guesses: &[(String, String)], target: &str) -> Option<usize> {
        ...
        for (i, (word, _player_id)) in guesses.iter().enumerate() {
            ...
            let correct_count = letter_results.iter()
                .filter(|lr| matches!(lr.status, LetterStatus::Correct))
                .count();
            
            let present_count = letter_results.iter()
...
mod tests {
    ...
    fn test_evaluate_guess_correct_word() {
        ...
        assert!(letters.iter().all(|l| matches!(l.status, LetterStatus::Correct)));
    ...
    fn test_duplicate_letter_handling_detailed() {
        ...
        assert!(
            matches!(letters[1].status, LetterStatus::Present | LetterStatus::Absent), 
            "{}", pos1_msg
        );
        ...
        let present_l_count = letters.iter().filter(|l| 
            l.letter == "l" && matches!(l.status, LetterStatus::Present)
        ).count();
    ...
    fn test_scoring_edge_cases() {
        ...
        assert_eq!(points, 0);
        assert_eq!(letters.len(), 5);
        assert!(letters.iter().all(|l| matches!(l.status, LetterStatus::Absent)));
    }
    ...
    fn test_duplicate_letter_handling() {
        ...
        let l_results: Vec<_> = letters2.iter().enumerate()
            .map(|(i, l)| (i, l.letter.clone(), l.status.clone()))
            .collect();
        
        let l_correct_or_present = letters2.iter()
        ...
        assert!(
            ...
            l_correct_or_present, l_results
        );
    }
    ...
    fn test_previous_guesses_affect_scoring() {
        ...
        let previous_guess = GuessResult {
            ...
            letters: vec![
                LetterResult { letter: "w".to_string(), status: LetterStatus::Absent, position: 0 },
                LetterResult { letter: "o".to_string(), status: LetterStatus::Present, position: 1 },
                LetterResult { letter: "r".to_string(), status: LetterStatus::Absent, position: 2 },
                LetterResult { letter: "l".to_string(), status: LetterStatus::Correct, position: 3 },
                LetterResult { letter: "d".to_string(), status: LetterStatus::Absent, position: 4 },
            ],
            points_earned: 3,
    ...
    fn test_game_rules_compliance() {
        ...
        let (letters, points) = ScoringEngine::evaluate_guess("hello", "hello", &[]);
        assert!(letters.iter().all(|l| matches!(l.status, LetterStatus::Correct))); // All blue
    ...
    fn test_no_points_for_already_revealed_letters() {
        ...
        let previous_guess = GuessResult {
            ...
            player_id: uuid::Uuid::new_v4(),
            letters: vec![
                LetterResult { letter: "h".to_string(), status: LetterStatus::Correct, position: 0 },
                LetterResult { letter: "o".to_string(), status: LetterStatus::Present, position: 1 },
                LetterResult { letter: "a".to_string(), status: LetterStatus::Absent, position: 2 },
                LetterResult { letter: "r".to_string(), status: LetterStatus::Absent, position: 3 },
                LetterResult { letter: "d".to_string(), status: LetterStatus::Absent, position: 4 },
            ],
