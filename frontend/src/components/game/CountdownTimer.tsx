    ...
    switch (gamePhase) {
      ...
      case "IndividualGuess":
        return currentWinnerName 
          ? `${currentWinnerName} is making their Bonus Guess`
          : "Winner is making their Bonus Guess";
      case "GameOver":
        return "Game Over!";
      default:
        return "Ready";
        ...
        {(gamePhase === "Guessing" ||
          ...
          timeLeft > 0 && (
            <div className="flex items-center gap-2">
              ...
              <span
                className={`font-mono font-bold text-2xl ${timeLeft <= 5 ? "text-red-600 animate-pulse" : "text-gray-800"}`}
              >
                {formatTime(timeLeft)}
              </span>
            </div>
        ...
        <div className="border-t pt-3 mt-2">
          ...
          <div className="flex justify-center gap-4 text-xs">
            <div className="flex items-center gap-1">
              ...
              <span className="text-gray-600">Correct position</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 bg-orange-500 rounded"></div>
              <span className="text-gray-600">In word, wrong spot</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 bg-gray-400 rounded"></div>
              <span className="text-gray-600">Not in word</span>
            </div>
          </div>
        </div>
      )}
...
export const CountdownTimerContainer: React.FC = () => {
  ...
  // Use the actual game phase from state
  ...
  // Get current winner's name for individual guess phase
  const currentWinnerName = gameState.current_winner
    ? gameState.players.find(p => p.user_id === gameState.current_winner)?.display_name
    : undefined;
