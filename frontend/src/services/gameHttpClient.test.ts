...
describe("GameHttpClient", () => {
  ...
  describe("getGameState", () => {
    ...
    it("should throw error for non-ok response", async () => {
      ...
      await expect(
        gameHttpClient.getGameState("nonexistent-game"),
      ).rejects.toThrow("Failed to fetch game state: 404 Not Found");
    });
    ...
    it("should handle special characters in gameId", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        `/api/game/${gameIdWithSpecialChars}/state`,
      );
    });
  ...
  describe("error message formatting", () => {
    it("should include status code and status text in error message", async () => {
      ...
      try {
        ...
        expect((error as Error).message).toBe(
          "Failed to fetch game state: 403 Forbidden",
        );
      }
    });
    ...
    it("should handle missing status text", async () => {
      ...
      try {
        ...
        expect((error as Error).message).toBe(
          "Failed to fetch game state: 418 ",
        );
      }
    });
  });
